...
// ===== BIẾN MỚI CHO CƯỢC ẢO VÀ CHẾ ĐỘ =====
var VIRTUAL_BUY_AMOUNT = 0;
var VIRTUAL_SELL_AMOUNT = 0;
var VIRTUAL_BETS_MODE = 'one_session'; // Các chế độ: 'one_session', 'multi_session'
// =============================================
...
wss.on('connection', function(ws){
    ...
    totalBuy = PRICE_BUY_LIVE + VIRTUAL_BUY_AMOUNT; // Cập nhật volume ảo
    totalSell = PRICE_SELL_LIVE + VIRTUAL_SELL_AMOUNT; // Cập nhật volume ảo
    ...
    ws.on('message', d => {
        ...
        // ===== BẮT SỰ KIỆN CẬP NHẬT CƯỢC ẢO TỪ ADMIN =====
        if(data.type === 'ADJUST_VIRTUAL_BETS'){
            let obj = data.data;
            VIRTUAL_BUY_AMOUNT = obj.buy;
            VIRTUAL_SELL_AMOUNT = obj.sell;

            // ===== LOGIC MỚI: GỬI CẬP NHẬT TỨC THÌ CHO ADMIN =====
            let admin_ws = null;
            for(let u in users){
                if(users[u].uid === 'ADMIN_BO'){
                    admin_ws = users[u].ws;
                    break;
                }
            }
            ...
            if(admin_ws){
                const DATA_LIST_BE_CAU_SNAPSHOT = [];

                for(let key in BTC_USER_BUY){
                    let moneyAndActionBuy = BTC_USER_BUY[key];
                    let moneyAndAction = moneyAndActionBuy.split("||");
                    if(moneyAndAction[2] == 1){
                        DATA_LIST_BE_CAU_SNAPSHOT.push({
                            e: moneyAndAction[3], uid: moneyAndAction[5], sv: SEVER_GET, 
                            bet: moneyAndAction[1], amount: moneyAndAction[0], mkt: moneyAndAction[4]
                        });
                    }
                }
                for(let key in BTC_USER_SELL){
                    let moneyAndActionSell = BTC_USER_SELL[key];
                ...
                let totalPriceBUY = PRICE_BUY_LIVE + VIRTUAL_BUY_AMOUNT;
                let totalPriceSELL = PRICE_SELL_LIVE + VIRTUAL_SELL_AMOUNT;
                
                admin_ws.send(JSON.stringify({
                    type: 'truck', 
                    data: DATA_LIST_BE_CAU_SNAPSHOT, 
                    price_buy: totalPriceBUY,
                    price_sell: totalPriceSELL, 
                    mktBUY: PRICE_MAKETING_BUY,
                    mktSELL: PRICE_MAKETING_SELL
                }));
            }
            // ==========================================================

            // Gửi cho tất cả user khác thấy thanh volume thay đổi
            let totalBuy = PRICE_BUY_LIVE + VIRTUAL_BUY_AMOUNT;
            let totalSell = PRICE_SELL_LIVE + VIRTUAL_SELL_AMOUNT;
            let total = totalBuy + totalSell;
            let currentTotalPTBuy = 50;
            let currentTotalPTSell = 50;

            if(total > 0){
                currentTotalPTBuy = toFixed((totalBuy/total)*100, 0);
                currentTotalPTSell = 100 - Number(currentTotalPTBuy);
            }

            wss.clients.forEach(function each(client) {
                let json = {nbuy: totalBuy, nsell: totalSell, ptbuy: Number(currentTotalPTBuy), ptsell: Number(currentTotalPTSell)}
                client.send(JSON.stringify({type: 'transVolum', data: json}));
            });
        }
        
        // ===== BẮT SỰ KIỆN ĐẶT CHẾ ĐỘ BƠM TIỀN =====
        if(data.type === 'SET_VIRTUAL_BETS_MODE'){
            VIRTUAL_BETS_MODE = data.data.mode;
        }
        // =============================================
...
function XU_LY_VOLUM(s, jDATA){
	...
	//	ORDER_OR_WATTING === 'order' && s > rdSe2 ||y
	...
	//){
       //console.log(ORDER_OR_WATTING + ' --- ' + s);
        ...
        // ===== THAY ĐỔI LOGIC, TÍNH CẢ CƯỢC ẢO =====
        let totalBuy = 0;
        let totalSell = 0;
        ...
        if(s < rdSe){
            ...
            totalBuy = PRICE_BUY_LIVE_BACKUP + VIRTUAL_BUY_AMOUNT;
            totalSell = PRICE_SELL_LIVE_BACKUP + VIRTUAL_SELL_AMOUNT;
        }
        if(s > rdSe2){
            totalBuy = PRICE_BUY_LIVE_BACKUP + VIRTUAL_BUY_AMOUNT;
            totalSell = PRICE_SELL_LIVE_BACKUP + VIRTUAL_SELL_AMOUNT;
    ...
    if(DATA_LIST_BE_CAU.length != 0){
        for(let obj in users) {
            ...
            if(uid == 'ADMIN_BO'){
                ...
                // ===== SỬA LỖI TẠI ĐÂY =====
                // Cộng thêm tiền ảo vào tổng tiền trước khi gửi
                let totalPriceBUY = PRICE_BUY_LIVE + VIRTUAL_BUY_AMOUNT;
                let totalPriceSELL = PRICE_SELL_LIVE + VIRTUAL_SELL_AMOUNT;
                // ============================
...
async function HandlingBuySell2(title){
    ...
    // ===== RESET CƯỢC ẢO SAU MỖI PHIÊN (TÙY THEO CHẾ ĐỘ) =====
    if (VIRTUAL_BETS_MODE === 'one_session') {
        VIRTUAL_BUY_AMOUNT = 0;
        VIRTUAL_SELL_AMOUNT = 0;
    }
    // ============================================================

    // ===== GỬI TIN NHẮN XÓA DỮ LIỆU CŨ CHO ADMIN KHI PHIÊN MỚI BẮT ĐẦU =====
    let admin_ws = null;
    for(let u in users){
        if(users[u].uid === 'ADMIN_BO'){
            admin_ws = users[u].ws;
    ...
    if(admin_ws){
        admin_ws.send(JSON.stringify({
            type: 'truck', 
            data: [], // Dữ liệu rỗng
            price_buy: VIRTUAL_BUY_AMOUNT, // Giữ lại tiền ảo nếu ở chế độ nhiều phiên
            price_sell: VIRTUAL_SELL_AMOUNT, // Giữ lại tiền ảo nếu ở chế độ nhiều phiên
            mktBUY: 0,
            mktSELL: 0
        }));
    }
...
function BOTAOStart(){
    ...
    startBotAo = setInterval(function(){
        ...
        let getPRICE_BUY = PRICE_BUY_LIVE + numberBuy + VIRTUAL_BUY_AMOUNT;
        let getPRICE_SELL = PRICE_SELL_LIVE + numberSell + VIRTUAL_SELL_AMOUNT;
